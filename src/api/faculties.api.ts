...
export const facultiesApi = {
  ...
  // Get faculty detail
  getFacultyDetail: async (id: string, signal?: AbortSignal): Promise<FacultyDetail> => {
    const response = await apiClient.get<Wrapped<FacultyDetail>>(`${BASE_URL}/${id}`, { signal })
    return response.data.data
  },

  // Get dictionaries
  getDictionaries: async (signal?: AbortSignal): Promise<FacultyDictionaries> => {
    const response = await apiClient.get<Wrapped<FacultyDictionaries>>(`${BASE_URL}/dictionaries`, {
      signal,
    })
    return response.data.data
  },
