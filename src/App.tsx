...
// Placeholder page for new routes. Takes an i18n KEY (not a pre-translated string) so the
// route tree stays independent of `t` — letting the router memo use empty deps — and the
// title still re-localizes on language change (this component re-renders with a fresh `t`).
const PlaceholderPage = ({ title }: { title: string }) => {
  const { t } = useTranslation()
  return (
    <div className="flex h-64 items-center justify-center">
      <div className="text-center">
        <h2 className="text-color-primary mb-2 text-xl font-semibold">{t(title)}</h2>
...
function App() {
  ...
  // Data router (createBrowserRouter) — required for useBlocker / loaders / data APIs.
  // Built once (useMemo, [] deps): the route tree references only stable module-scope
  // components, so it never needs to rebuild across auth-driven re-renders.
                ...
                <Route
                  ...
                  element={
                    <ProtectedRoute permission="system.report-update.view">
                      <PlaceholderPage title="Report updates" />
                    </ProtectedRoute>
                  }
                />
              </Route>
