...
interface ClassifierTablePanelProps {
  /**
   * Backend classifier metadata (apiKey, editable, hierarchical, ...).
   * All CRUD affordances are driven by `classifier.editable` — never hardcode.
   */
  classifier: ClassifierMetadata
  /** Show the classifier title/table-name/badges header block. Default true. */
  showHeader?: boolean
}

/**
 * Full LIST + SEARCH + CREATE + EDIT + soft-DELETE panel scoped to a SINGLE
 * classifier (by `classifier.apiKey`). Reused by both the generic category page
 * and the dedicated deep-link pages (Positions, Qualifications).
 *
 * URL state (`page`, `size`, `q`) is namespaced-free — mount one panel per route.
 * Remount (via a `key={apiKey}` on the parent) resets all internal state cleanly.
 */
export default function ClassifierTablePanel({
  classifier,
  showHeader = true,
}: ClassifierTablePanelProps) {
  const { t } = useTranslation()
  const [searchParams, setSearchParams] = useSearchParams()

  const apiKey = classifier.apiKey
  ...
  const { data: itemsData, isLoading: isLoadingItems } = useClassifierItems(apiKey, {
    page: currentPage,
    size: pageSize,
    search: debouncedSearch || undefined,
  })

  const createMutation = useCreateClassifierItem(apiKey)
  const updateMutation = useUpdateClassifierItem(apiKey)
  const deleteMutation = useDeleteClassifierItem(apiKey)
  ...
  const colCount = classifier.editable ? 8 : 7
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <Dialog open={showCreateForm} onOpenChange={setShowCreateForm}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {t('Add')} — {classifier.titleUz}
            </DialogTitle>
            <DialogDescription>{classifier.tableName}</DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{classifier.titleUz}</DialogDescription>
          </DialogHeader>
          <div className="grid gap-3">
            <div>
              <label className="mb-1 block text-sm font-medium text-[var(--text-primary)]">
      ...
      {totalElements > 0 && (
        <DataTablePagination
          ...
          pageSize={pageSize}
          onPageChange={(page) => updateSearchParams({ page: page > 0 ? String(page) : undefined })}
          onPageSizeChange={(size) => updateSearchParams({ size: String(size), page: undefined })}
        />
      )}
    </div>
  )
