...
import { Download, RefreshCw, Eye, Search, FlaskConical, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import {
  useResearchers,
  useResearcherDictionaries,
  useExportResearchers,
} from '@/hooks/useResearchers'
...
export default function ResearchersPage() {
  ...
  const scienceBranchFromUrl = searchParams.get('scienceBranch') || 'all'
  const typeFromUrl = searchParams.get('doctoralStudentType') || 'all'
  ...
  const { data: dictionaries } = useResearcherDictionaries()
  ...
  const handleScienceBranchChange = useCallback(
    (value: string) => {
      updateSearchParams({ scienceBranch: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )
  ...
  const handleTypeChange = useCallback(
    (value: string) => {
      updateSearchParams({
        doctoralStudentType: value !== 'all' ? value : undefined,
        page: undefined,
      })
    },
    [updateSearchParams],
  ...
  const handleStatusChange = useCallback(
    (value: string) => {
      updateSearchParams({ status: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useResearchers({
    search: debouncedSearch,
    universityCode: universityFromUrl,
    scienceBranch: scienceBranchFromUrl,
    doctoralStudentType: typeFromUrl,
  ...
  const exportMutation = useExportResearchers()

  const handleExport = useCallback(() => {
    exportMutation.mutate({
      q: debouncedSearch || undefined,
      universityCode: universityFromUrl === 'all' ? undefined : universityFromUrl,
      scienceBranch: scienceBranchFromUrl === 'all' ? undefined : scienceBranchFromUrl,
      doctoralStudentType: typeFromUrl === 'all' ? undefined : typeFromUrl,
  ...
  return (
    <>
      <div className="space-y-3 p-4">
        ...
        <div className="rounded-md border border-[var(--border-color-pro)] bg-[var(--card-bg)] shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-color-pro)] px-4 py-2.5">
            ...
            <Select value={scienceBranchFromUrl} onValueChange={handleScienceBranchChange}>
              <SelectTrigger className="w-[170px]">
                <SelectValue placeholder={t('Science branch')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('Science branch')}</SelectItem>
                {dictionaries?.scienceBranches?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
            ...
            <Select value={typeFromUrl} onValueChange={handleTypeChange}>
              <SelectTrigger className="w-[170px]">
                <SelectValue placeholder={t('Doctoral student type')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('Doctoral student type')}</SelectItem>
                {dictionaries?.doctoralStudentTypes?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
            ...
            <Select value={statusFromUrl} onValueChange={handleStatusChange}>
              ...
              <SelectContent>
                ...
                {dictionaries?.statuses?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <FlaskConical className="h-8 w-8 text-[var(--text-secondary)]" />
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No researchers have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
                            onClick={() => {
                              ...
                              updateSearchParams({
                                ...
                                scienceBranch: undefined,
                                doctoralStudentType: undefined,
                                status: undefined,
                                active: undefined,
                                page: undefined,
                              })
                  ...
                  rows.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-sm text-[var(--text-secondary)]">
                        {row.scienceBranchName || '-'}
                      </td>
                      <td className="px-3 py-2 text-sm text-[var(--text-secondary)]">
                        {row.doctoralStudentTypeName || '-'}
                      </td>
                      <td className="px-3 py-2 text-sm text-[var(--text-secondary)]">
                        {row.acceptedDate || '-'}
                      </td>
      ...
      {selectedId && (
        <ResearcherDetailDrawer researcherId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
