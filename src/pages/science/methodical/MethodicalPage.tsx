...
import { Download, RefreshCw, Eye, Search, BookCopy, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import {
  useMethodical,
  useMethodicalDictionaries,
  useExportMethodical,
} from '@/hooks/useMethodical'
...
export default function MethodicalPage() {
  ...
  const methodicalTypeFromUrl = searchParams.get('methodicalType') || 'all'
  ...
  const { data: dictionaries } = useMethodicalDictionaries()
  ...
  const handleMethodicalTypeChange = useCallback(
    (value: string) => {
      updateSearchParams({ methodicalType: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useMethodical({
    search: debouncedSearch,
    universityCode: universityFromUrl,
    methodicalType: methodicalTypeFromUrl,
  ...
  const exportMutation = useExportMethodical()

  const handleExport = useCallback(() => {
    exportMutation.mutate({
      q: debouncedSearch || undefined,
      universityCode: universityFromUrl === 'all' ? undefined : universityFromUrl,
      methodicalType: methodicalTypeFromUrl === 'all' ? undefined : methodicalTypeFromUrl,
  ...
  return (
    <>
      <div className="space-y-3 p-4">
        ...
        <div className="rounded-md border border-[var(--border-color-pro)] bg-[var(--card-bg)] shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-color-pro)] px-4 py-2.5">
            ...
            <Select value={methodicalTypeFromUrl} onValueChange={handleMethodicalTypeChange}>
              <SelectTrigger className="w-[170px]">
                <SelectValue placeholder={t('Type')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('Type')}</SelectItem>
                {dictionaries?.methodicalTypes?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <BookCopy className="h-8 w-8 text-[var(--text-secondary)]" />
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No methodical works have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
      ...
      {selectedId && (
        <MethodicalDetailDrawer methodicalId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
