...
import {
  useDissertationDefense,
  useDissertationDefenseDictionaries,
  useExportDissertationDefense,
} from '@/hooks/useDissertationDefense'
...
export default function DissertationDefensePage() {
  ...
  const { data: dictionaries } = useDissertationDefenseDictionaries()
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useDissertationDefense({
  ...
  const exportMutation = useExportDissertationDefense()
  ...
  }, [exportMutation, debouncedSearch, universityFromUrl, activeFromUrl])
  ...
  const hasFilters = !!(debouncedSearch || universityFromUrl !== 'all' || activeFromUrl !== 'all')

  return (
    <>
      <div className="space-y-3 p-4">
        <h1 className="font-display text-xl font-semibold text-[var(--text-primary)]">
          {t('Dissertation defense')}
        </h1>
      ...
      {selectedId && (
        <DissertationDefenseDetailDrawer
          defenseId={selectedId}
