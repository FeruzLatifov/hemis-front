...
import { Download, RefreshCw, Eye, Search, Lightbulb, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import {
  useIntellectual,
  useIntellectualDictionaries,
  useExportIntellectual,
} from '@/hooks/useIntellectual'
...
export default function IntellectualPropertyPage() {
  ...
  const patentTypeFromUrl = searchParams.get('patentType') || 'all'
  ...
  const { data: dictionaries } = useIntellectualDictionaries()
  ...
  const handlePatentTypeChange = useCallback(
    (value: string) => {
      updateSearchParams({ patentType: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useIntellectual({
    search: debouncedSearch,
    universityCode: universityFromUrl,
    patentType: patentTypeFromUrl,
  ...
  const exportMutation = useExportIntellectual()

  const handleExport = useCallback(() => {
    exportMutation.mutate({
      q: debouncedSearch || undefined,
      universityCode: universityFromUrl === 'all' ? undefined : universityFromUrl,
      patentType: patentTypeFromUrl === 'all' ? undefined : patentTypeFromUrl,
      active: activeFromUrl === 'all' ? undefined : activeFromUrl === 'true',
    })
  }, [exportMutation, debouncedSearch, universityFromUrl, patentTypeFromUrl, activeFromUrl])
  ...
  return (
    <>
      <div className="space-y-3 p-4">
        ...
        <div className="rounded-md border border-[var(--border-color-pro)] bg-[var(--card-bg)] shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-color-pro)] px-4 py-2.5">
            ...
            <Select value={patentTypeFromUrl} onValueChange={handlePatentTypeChange}>
              <SelectTrigger className="w-[170px]">
                <SelectValue placeholder={t('Patent type')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('Patent type')}</SelectItem>
                {dictionaries?.patentTypes?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <Lightbulb className="h-8 w-8 text-[var(--text-secondary)]" />
                        <div>
      ...
      {selectedId && (
        <IntellectualPropertyDetailDrawer
