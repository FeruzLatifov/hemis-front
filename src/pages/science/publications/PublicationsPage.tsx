...
import { Download, RefreshCw, Eye, Search, BookMarked, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import {
  usePublications,
  usePublicationDictionaries,
  useExportPublications,
...
export default function PublicationsPage() {
  ...
  const publicationTypeFromUrl = searchParams.get('publicationType') || 'all'
  const issueYearFromUrl = searchParams.get('issueYear') || ''
  ...
  const [issueYearInput, setIssueYearInput] = useState(issueYearFromUrl)
  const debouncedIssueYear = useDebounce(issueYearInput, UI.SEARCH_DEBOUNCE)
  const [selectedId, setSelectedId] = useState<string | null>(null)

  const { data: dictionaries } = usePublicationDictionaries()
  ...
  const syncIssueYearToUrl = useStableCallback(() => {
    if (debouncedIssueYear !== issueYearFromUrl) {
      updateSearchParams({ issueYear: debouncedIssueYear || undefined, page: undefined })
    }
  })

  useEffect(() => {
    syncIssueYearToUrl()
  }, [debouncedIssueYear, syncIssueYearToUrl])
  ...
  const handlePublicationTypeChange = useCallback(
    (value: string) => {
      updateSearchParams({ publicationType: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )
  ...
  const { data, isLoading, isPlaceholderData, refetch } = usePublications({
    search: debouncedSearch,
    universityCode: universityFromUrl,
    publicationType: publicationTypeFromUrl,
    issueYear: debouncedIssueYear,
  ...
  const exportMutation = useExportPublications()

  const handleExport = useCallback(() => {
    exportMutation.mutate({
      q: debouncedSearch || undefined,
      universityCode: universityFromUrl === 'all' ? undefined : universityFromUrl,
      publicationType: publicationTypeFromUrl === 'all' ? undefined : publicationTypeFromUrl,
      issueYear: debouncedIssueYear || undefined,
  ...
  return (
    <>
      <div className="space-y-3 p-4">
        ...
        <div className="rounded-md border border-[var(--border-color-pro)] bg-[var(--card-bg)] shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-color-pro)] px-4 py-2.5">
            ...
            <Select value={publicationTypeFromUrl} onValueChange={handlePublicationTypeChange}>
              <SelectTrigger className="w-[170px]">
                <SelectValue placeholder={t('Publication type')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('Publication type')}</SelectItem>
                {dictionaries?.publicationTypes?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
            ...
            <input
              type="text"
              inputMode="numeric"
              placeholder={t('Issue year')}
              value={issueYearInput}
              onChange={(e) => setIssueYearInput(e.target.value)}
              className="w-[110px] rounded-md border border-[var(--input-border)] bg-[var(--input-bg)] px-3 py-1.5 text-sm text-[var(--text-primary)] transition-colors placeholder:text-[var(--text-secondary)] focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--active-bg)] focus:outline-none"
            />
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <BookMarked className="h-8 w-8 text-[var(--text-secondary)]" />
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No publications have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
      ...
      {selectedId && (
        <PublicationDetailDrawer publicationId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
