...
import { Download, RefreshCw, Eye, Search, Beaker, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import {
  useScientificProjects,
  useScientificProjectDictionaries,
  useExportScientificProjects,
} from '@/hooks/useScientificProjects'
...
export default function ScientificProjectsPage() {
  ...
  const { data: dictionaries } = useScientificProjectDictionaries()
  ...
  const handleProjectTypeChange = useCallback(
    (value: string) => {
      updateSearchParams({ projectType: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useScientificProjects({
  ...
  const exportMutation = useExportScientificProjects()

  const handleExport = useCallback(() => {
    exportMutation.mutate({
      q: debouncedSearch || undefined,
      universityCode: universityFromUrl === 'all' ? undefined : universityFromUrl,
      projectType: projectTypeFromUrl === 'all' ? undefined : projectTypeFromUrl,
      active: activeFromUrl === 'all' ? undefined : activeFromUrl === 'true',
    })
  }, [exportMutation, debouncedSearch, universityFromUrl, projectTypeFromUrl, activeFromUrl])
  ...
  return (
    <>
      <div className="space-y-3 p-4">
        ...
        <div className="rounded-md border border-[var(--border-color-pro)] bg-[var(--card-bg)] shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-color-pro)] px-4 py-2.5">
            ...
            <Select value={projectTypeFromUrl} onValueChange={handleProjectTypeChange}>
              <SelectTrigger className="w-[170px]">
                <SelectValue placeholder={t('Project type')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('Project type')}</SelectItem>
                {dictionaries?.projectTypes?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <Beaker className="h-8 w-8 text-[var(--text-secondary)]" />
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No scientific projects have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
      ...
      {selectedId && (
        <ScientificProjectDetailDrawer projectId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
