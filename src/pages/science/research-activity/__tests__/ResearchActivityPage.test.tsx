...
vi.mock('@/api/researchActivity.api', () => ({
  researchActivityApi: {
    ...
    getDictionaries: vi.fn().mockResolvedValue({
      ...
      scholarDatabases: [{ code: 'SD1', name: 'Scopus' }],
    }),
    export: vi.fn().mockResolvedValue(new Blob(['test'], { type: 'text/csv' })),
  },
}))

vi.mock('../ResearchActivityDetailDrawer', () => ({
  default: ({ activityId, onClose }: { activityId: string; onClose: () => void }) => (
    <div data-testid="research-activity-detail-drawer">
      Detail: {activityId}
...
import ResearchActivityPage from '../ResearchActivityPage'
import { researchActivityApi } from '@/api/researchActivity.api'

describe('ResearchActivityPage', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('renders the search input', async () => {
    render(<ResearchActivityPage />)
    await waitFor(() => {
  ...
  it('shows a research activity row from the fixture', async () => {
    render(<ResearchActivityPage />)
    await waitFor(() => {
  ...
  it('shows the empty state when there are no rows', async () => {
    ;(researchActivityApi.list as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      content: [],
      totalElements: 0,
      totalPages: 0,
      size: 20,
  ...
  it('triggers the export mutation when Export is clicked', async () => {
    ...
    await waitFor(() => {
      expect(researchActivityApi.export).toHaveBeenCalled()
    })
  })
})
