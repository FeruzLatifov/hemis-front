...
import {
  useResearchActivity,
  useResearchActivityDictionaries,
  useExportResearchActivity,
} from '@/hooks/useResearchActivity'
...
export default function ResearchActivityPage() {
  ...
  const { data: dictionaries } = useResearchActivityDictionaries()
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useResearchActivity({
  ...
  const exportMutation = useExportResearchActivity()
  ...
  }, [exportMutation, debouncedSearch, universityFromUrl, educationYearFromUrl])
  ...
  return (
    <>
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-color-pro)] px-4 py-2.5">
            ...
            <Select value={educationYearFromUrl} onValueChange={handleEducationYearChange}>
              ...
              <SelectContent>
                ...
                {dictionaries?.scholarDatabases?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
      ...
      {selectedId && (
        <ResearchActivityDetailDrawer activityId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
