...
import { Download, RefreshCw, Eye, Search, Award, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import {
  useCertificates,
  useCertificateDictionaries,
  useExportCertificates,
...
export default function CertificatesPage() {
  ...
  const typeFromUrl = searchParams.get('certificateType') || 'all'

  const [searchInput, setSearchInput] = useState(searchFromUrl)
  const debouncedSearch = useDebounce(searchInput, UI.SEARCH_DEBOUNCE)
  const [selectedId, setSelectedId] = useState<string | null>(null)

  const { data: dictionaries } = useCertificateDictionaries()
  ...
  const handleTypeChange = useCallback(
    (value: string) => {
      updateSearchParams({ certificateType: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )

  const { data, isLoading, isPlaceholderData, refetch } = useCertificates({
    search: debouncedSearch,
    universityCode: universityFromUrl,
    certificateType: typeFromUrl,
    page: currentPage,
  })
  ...
  const handleExport = useCallback(() => {
    exportMutation.mutate({
      ...
      certificateType: typeFromUrl === 'all' ? undefined : typeFromUrl,
    })
  }, [exportMutation, debouncedSearch, universityFromUrl, typeFromUrl])
  ...
  const hasFilters = !!(debouncedSearch || universityFromUrl !== 'all' || typeFromUrl !== 'all')
  ...
  return (
    <>
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-color-pro)] px-4 py-2.5">
            ...
            <Select value={typeFromUrl} onValueChange={handleTypeChange}>
              <SelectTrigger className="w-[180px]">
                <SelectValue placeholder={t('Certificate type')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="all">{t('Certificate type')}</SelectItem>
                {dictionaries?.certificateTypes?.map((item) => (
                  <SelectItem key={item.code} value={item.code}>
                    {item.name}
                  </SelectItem>
                ))}
                ...
                {loading ? (
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        <Award className="h-8 w-8 text-[var(--text-secondary)]" />
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No certificates have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
                  ...
                  rows.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-sm text-[var(--text-secondary)]">
                        {row.certificateTypeName || row.certificateType || '-'}
                      </td>
                      <td className="px-3 py-2 text-sm text-[var(--text-secondary)]">
                        {row.certificateNameLabel || row.certificateName || '-'}
                      </td>
                      <td className="px-3 py-2 text-sm text-[var(--text-secondary)]">
                        {row.serialNumber || '-'}
                      </td>
      ...
      {selectedId && (
        <CertificateDetailDrawer certificateId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
