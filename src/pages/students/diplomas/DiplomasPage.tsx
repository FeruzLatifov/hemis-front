...
import { Download, RefreshCw, Eye, Search, GraduationCap, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import { useDiplomas, useDiplomaDictionaries, useExportDiplomas } from '@/hooks/useDiplomas'
...
export default function DiplomasPage() {
  ...
  const verifyFromUrl = searchParams.get('verify') || 'all'

  const [searchInput, setSearchInput] = useState(searchFromUrl)
  const debouncedSearch = useDebounce(searchInput, UI.SEARCH_DEBOUNCE)
  const [selectedId, setSelectedId] = useState<string | null>(null)

  const { data: dictionaries } = useDiplomaDictionaries()
  ...
  const handleEducationYearChange = useCallback(
    (value: string) => {
      updateSearchParams({ educationYear: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )
  ...
  const handleVerifyChange = useCallback(
    (value: string) => {
      updateSearchParams({ verify: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )

  const { data, isLoading, isPlaceholderData, refetch } = useDiplomas({
    search: debouncedSearch,
    universityCode: universityFromUrl,
    educationYear: educationYearFromUrl,
    verify: verifyFromUrl,
    page: currentPage,
  })
  ...
  const exportMutation = useExportDiplomas()
  ...
  const handleExport = useCallback(() => {
    exportMutation.mutate({
      ...
      verify: verifyFromUrl === 'all' ? undefined : verifyFromUrl === 'true',
    })
  }, [exportMutation, debouncedSearch, universityFromUrl, educationYearFromUrl, verifyFromUrl])
  ...
  return (
    <>
      <div className="space-y-3 p-4">
        <div className="rounded-md border border-[var(--border-color-pro)] bg-[var(--card-bg)] shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border-color-pro)] px-4 py-2.5">
            ...
            <Select value={verifyFromUrl} onValueChange={handleVerifyChange}>
              <SelectTrigger className="w-[140px]">
                <SelectValue placeholder={t('Verified')} />
              </SelectTrigger>
              <SelectContent>
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No diplomas have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
      ...
      {selectedId && (
        <DiplomaDetailDrawer diplomaId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
