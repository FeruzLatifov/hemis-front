...
import { Download, RefreshCw, Eye, Search, Wallet, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import {
  useScholarships,
  useScholarshipDictionaries,
  useExportScholarships,
...
export default function ScholarshipsPage() {
  ...
  const categoryFromUrl = searchParams.get('stipendCategory') || 'all'

  const [searchInput, setSearchInput] = useState(searchFromUrl)
  const debouncedSearch = useDebounce(searchInput, UI.SEARCH_DEBOUNCE)
  const [selectedId, setSelectedId] = useState<string | null>(null)

  const { data: dictionaries } = useScholarshipDictionaries()
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useScholarships({
    search: debouncedSearch,
    universityCode: universityFromUrl,
    educationYear: educationYearFromUrl,
    stipendCategory: categoryFromUrl,
    page: currentPage,
  })
  ...
  const exportMutation = useExportScholarships()
  ...
  const handleExport = useCallback(() => {
    exportMutation.mutate({
      ...
      stipendCategory: categoryFromUrl === 'all' ? undefined : categoryFromUrl,
    })
  }, [exportMutation, debouncedSearch, universityFromUrl, educationYearFromUrl, categoryFromUrl])
  ...
  return (
    <>
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No scholarships have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
      ...
      {selectedId && (
        <ScholarshipDetailDrawer scholarshipId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
