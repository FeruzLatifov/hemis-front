...
import { type DepartmentGroupRow, type DepartmentRow } from '@/api/departments.api'
import {
  useDepartmentGroups,
  useDepartmentsByUniversity,
...
export default function DepartmentsPage() {
  ...
  // Fetch departments for expanded universities
  const expandedUniversities = Object.keys(expanded).filter((key) => expanded[key])

  const departmentQueries = useDepartmentsByUniversity(expandedUniversities, {
    search: debouncedSearch,
    status: statusFromUrl,
  })
  ...
  const rows = useMemo(() => {
    ...
    const result: Array<
      | { type: 'group'; data: DepartmentGroupRow }
      | { type: 'department'; data: DepartmentRow; universityCode: string }
    > = []

    for (const group of groupsData.content) {
      result.push({ type: 'group', data: group })

      if (expanded[group.universityCode] && departmentQueries.data?.[group.universityCode]) {
        const departments = departmentQueries.data[group.universityCode].content
    ...
    return result
  }, [groupsData, expanded, departmentQueries.data])
              ...
              <tbody>
                {loading ? (
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No departments have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
                  ...
                  rows.map((row, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2 text-sm">
                            <span className="font-medium">{group.departmentCount}</span>
                            <span className="ml-1 text-[var(--text-secondary)]">
                              ({t('Active')}: {group.activeDepartmentCount} / {t('Inactive')}:{' '}
                              {group.inactiveDepartmentCount})
                            </span>
                          </td>
                          <td className="px-3 py-2" />
                          <td className="px-3 py-2" />
                    ...
                    // Department row (child — indented, subtle background)
                    const department = row.data
                    return (
                      <tr
