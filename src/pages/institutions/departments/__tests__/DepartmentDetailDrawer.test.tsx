...
vi.mock('@/api/departments.api', () => ({
  departmentsApi: {
    getDepartmentDetail: vi.fn().mockResolvedValue({
      code: 'D001',
      nameUz: 'Fizika kafedrasi',
      nameRu: 'Department of Physics',
      shortName: 'Physics',
      universityCode: 'U001',
...
describe('DepartmentDetailDrawer', () => {
  const defaultProps = {
    departmentCode: 'D001',
    onClose: vi.fn(),
  }
  ...
  it('renders the drawer', async () => {
    render(<DepartmentDetailDrawer {...defaultProps} />)

    await waitFor(() => {
      expect(screen.getByText('Fizika kafedrasi')).toBeInTheDocument()
    })
  })

  it('shows department code', async () => {
    render(<DepartmentDetailDrawer {...defaultProps} />)
  ...
  it('shows loading skeletons when loading', async () => {
    const api = await import('@/api/departments.api')
    // Override to never resolve - simulating a pending state
    vi.mocked(api.departmentsApi.getDepartmentDetail).mockReturnValueOnce(new Promise(() => {}))
  ...
  it('shows error state on failure', async () => {
    const api = await import('@/api/departments.api')
    vi.mocked(api.departmentsApi.getDepartmentDetail).mockRejectedValueOnce(
