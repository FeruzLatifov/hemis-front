...
import { Download, RefreshCw, Eye, Search, FileText, Loader2 } from 'lucide-react'
import {
  Select,
  SelectContent,
  SelectItem,
...
import {
  useDiplomaBlanks,
  useDiplomaBlankDictionaries,
  useExportDiplomaBlanks,
} from '@/hooks/useDiplomaBlanks'
import { PAGINATION, UI } from '@/constants'
import { toast } from 'sonner'
import DiplomaBlankDetailDrawer from './DiplomaBlankDetailDrawer'

export default function DiplomaBlanksPage() {
  ...
  const { data: dictionaries } = useDiplomaBlankDictionaries()
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useDiplomaBlanks({
  ...
  const exportMutation = useExportDiplomaBlanks()
  ...
  }, [exportMutation, debouncedSearch, universityFromUrl, statusFromUrl])
  ...
  const hasFilters = !!(debouncedSearch || universityFromUrl !== 'all' || statusFromUrl !== 'all')
  ...
  return (
    <>
                        ...
                        <div>
                          ...
                          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
                            {hasFilters
                              ...
                              : t('No diploma blanks have been added yet')}
                          </p>
                        </div>
                        {hasFilters && (
                          <button
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        <Badge variant={row.active ? 'default' : 'secondary'}>
                          {row.statusCode || (row.active ? t('Active') : t('Inactive'))}
                        </Badge>
                      </td>
                      <td className="px-3 py-2">
                        <button
      ...
      {selectedId && (
        <DiplomaBlankDetailDrawer diplomaBlankId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
