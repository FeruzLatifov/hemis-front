...
import {
  useUniversitySpecialities,
  useUniversitySpecialityDictionaries,
  useExportUniversitySpecialities,
} from '@/hooks/useUniversitySpecialities'
...
export default function UniversitySpecialitiesPage() {
  ...
  const { data: dictionaries } = useUniversitySpecialityDictionaries()
  ...
  const handleEducationTypeChange = useCallback(
    (value: string) => {
      updateSearchParams({ educationType: value !== 'all' ? value : undefined, page: undefined })
    },
    [updateSearchParams],
  )
  ...
  const { data, isLoading, isPlaceholderData, refetch } = useUniversitySpecialities({
  ...
  const exportMutation = useExportUniversitySpecialities()
  ...
  }, [exportMutation, debouncedSearch, universityFromUrl, educationTypeFromUrl, activeFromUrl])
  ...
  return (
    <>
      ...
      {selectedId && (
        <UniversitySpecialityDetailDrawer
