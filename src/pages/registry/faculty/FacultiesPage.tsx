...
export default function FacultiesPage() {
  ...
  const { data: groupsData, isLoading, error, refetch } = useQuery({
    queryKey: ['faculty-groups', search, status, page],
    queryFn: async () => {
      try {
        const result = await facultiesApi.getGroups({
          q: search || undefined,
                  ...
                  {table.getRowModel().rows.map((row) => (
                    <React.Fragment key={row.id}>
                      ...
                      {row.original.subRows?.map((subRow, idx) => (
                        <tr key={`${row.id}-sub-${idx}`} className="border-b hover:bg-muted/50">
                          {columns.map((column, colIdx) => (
                            <td key={colIdx} className="px-4 py-2">
                              {column.cell && typeof column.cell === 'function'
                                ? flexRender(column.cell, { row: { original: subRow } } as never)
                                : null}
                            </td>
                          ))}
                        </tr>
