...
export default function UniversitiesPage() {
  ...
  // Horizontal columns state (stat-ministry style)
  const [horizontalFilters, setHorizontalFilters] = useState<Array<{key: string, label: string, values: string[]}>>([]);
  const [searchFilterText, setSearchFilterText] = useState<Record<string, string>>({});
  ...
  const loadUniversities = async () => {
    try {
      ...
      // Build filter params from horizontal filters
      const params: any = {
        page: currentPage,
        size: pageSize,
        q: search || undefined,
      ...
      // Add horizontal filter values
      horizontalFilters.forEach(filter => {
        if (filter.values.length > 0) {
          if (filter.key === 'region') {
            params.regionId = filter.values.join(',');
          } else if (filter.key === 'ownership') {
            params.ownershipId = filter.values.join(',');
          } else if (filter.key === 'type') {
            params.typeId = filter.values.join(',');
          } else {
            // For text/boolean filters, add them as query params
  ...
  }, [currentPage, pageSize, search, regionId, ownershipId, typeId, horizontalFilters]);
  ...
  const filterProperties: FilterProperty[] = [
    ...
    },
    {
      ...
      type: 'select',
      options: dictionaries.types.map((type) => ({ value: type.code, label: type.name })),
    },
  ...
  const activeFiltersCount = useMemo(() => {
    let count = 0;
    if (regionId) count++;
    if (ownershipId) count++;
    if (typeId) count++;
    count += horizontalFilters.length;
    return count;
  }, [regionId, ownershipId, typeId, horizontalFilters.length]);

  const handleAddHorizontalFilter = (key: string, label: string) => {
    if (!horizontalFilters.find(f => f.key === key)) {
      setHorizontalFilters(prev => [...prev, { key, label, values: [] }]);
    }
  };

  const handleRemoveHorizontalFilter = (key: string) => {
    setHorizontalFilters(horizontalFilters.filter(f => f.key !== key));
    setOpenPopover(null);
    const newSearchText = {...searchFilterText};
    delete newSearchText[key];
    setSearchFilterText(newSearchText);
  };

  const handleUpdateHorizontalFilterValues = (key: string, values: string[]) => {
    setHorizontalFilters(prev => prev.map(f => 
